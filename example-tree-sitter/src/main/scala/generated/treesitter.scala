...
object predef:
  private[treesitter] trait _BindgenEnumCUnsignedInt[T](using
      eq: T =:= CUnsignedInt
  ):
...
object enumerations:
  ...
  object TSQueryPredicateStepType
      extends _BindgenEnumCUnsignedInt[TSQueryPredicateStepType]:
    ...
    def getName(value: TSQueryPredicateStepType): Option[String] =
      value match
        case `TSQueryPredicateStepTypeDone` =>
          Some("TSQueryPredicateStepTypeDone")
        case `TSQueryPredicateStepTypeCapture` =>
          Some("TSQueryPredicateStepTypeCapture")
        case `TSQueryPredicateStepTypeString` =>
          Some("TSQueryPredicateStepTypeString")
        case _ => _root_.scala.None
    extension (a: TSQueryPredicateStepType)
      inline def &(b: TSQueryPredicateStepType): TSQueryPredicateStepType =
        a & b
      inline def |(b: TSQueryPredicateStepType): TSQueryPredicateStepType =
        a | b
      inline def is(b: TSQueryPredicateStepType): Boolean = (a & b) == b
  end TSQueryPredicateStepType
...
object aliases:
  ...
  object TSFieldId:
    ...
    extension (v: TSFieldId) inline def value: uint16_t = v
...
object structs:
  ...
  opaque type TSInput = CStruct3[Ptr[Byte], CFuncPtr4[Ptr[
    Byte
  ], uint32_t, TSPoint, Ptr[uint32_t], CString], TSInputEncoding]

  object TSInput:
    given _tag: Tag[TSInput] =
      Tag.materializeCStruct3Tag[Ptr[Byte], CFuncPtr4[Ptr[
        Byte
      ], uint32_t, TSPoint, Ptr[uint32_t], CString], TSInputEncoding]

    // Allocates TSInput on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSInput] =
      scala.scalanative.unsafe.alloc[TSInput](1)
    def apply(
        payload: Ptr[Byte],
        read: CFuncPtr4[Ptr[Byte], uint32_t, TSPoint, Ptr[uint32_t], CString],
        encoding: TSInputEncoding
    )(using Zone): Ptr[TSInput] =
      val ____ptr = apply()
    ...
    extension (struct: TSInput)
      ...
      def read
          : CFuncPtr4[Ptr[Byte], uint32_t, TSPoint, Ptr[uint32_t], CString] =
        struct._2
      def read_=(
          value: CFuncPtr4[Ptr[Byte], uint32_t, TSPoint, Ptr[uint32_t], CString]
  ...
  opaque type TSInputEdit =
    CStruct6[uint32_t, uint32_t, uint32_t, TSPoint, TSPoint, TSPoint]

  object TSInputEdit:
    given _tag: Tag[TSInputEdit] = Tag.materializeCStruct6Tag[
      uint32_t,
      uint32_t,
      uint32_t,
      TSPoint,
    ...
    def apply()(using Zone): Ptr[TSInputEdit] =
      scala.scalanative.unsafe.alloc[TSInputEdit](1)
    def apply(
        start_byte: uint32_t,
  ...
  opaque type TSLogger =
    CStruct2[Ptr[Byte], CFuncPtr3[Ptr[Byte], TSLogType, CString, Unit]]

  object TSLogger:
    given _tag: Tag[TSLogger] =
      Tag.materializeCStruct2Tag[Ptr[Byte], CFuncPtr3[Ptr[
        Byte
      ], TSLogType, CString, Unit]]

    // Allocates TSLogger on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSLogger] =
      scala.scalanative.unsafe.alloc[TSLogger](1)
    def apply(
        payload: Ptr[Byte],
        log: CFuncPtr3[Ptr[Byte], TSLogType, CString, Unit]
    )(using Zone): Ptr[TSLogger] =
      val ____ptr = apply()
      (!____ptr).payload = payload
      (!____ptr).log = log
      ____ptr
    ...
    extension (struct: TSLogger)
      ...
      def log_=(value: CFuncPtr3[Ptr[Byte], TSLogType, CString, Unit]): Unit =
        !struct.at2 = value
  end TSLogger
  ...
  opaque type TSNode =
    CStruct3[CArray[uint32_t, Nat._4], Ptr[Byte], Ptr[TSTree]]

  object TSNode:
    given _tag: Tag[TSNode] = Tag
      .materializeCStruct3Tag[CArray[uint32_t, Nat._4], Ptr[Byte], Ptr[TSTree]]

    // Allocates TSNode on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSNode] =
      scala.scalanative.unsafe.alloc[TSNode](1)
    def apply(
        context: CArray[uint32_t, Nat._4],
        id: Ptr[Byte],
        tree: Ptr[TSTree]
    )(using Zone): Ptr[TSNode] =
      val ____ptr = apply()
  ...
  object TSPoint:
    ...
    def apply()(using Zone): Ptr[TSPoint] =
      scala.scalanative.unsafe.alloc[TSPoint](1)
  ...
  object TSQueryCapture:
    given _tag: Tag[TSQueryCapture] =
      Tag.materializeCStruct2Tag[TSNode, uint32_t]

    // Allocates TSQueryCapture on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSQueryCapture] =
      scala.scalanative.unsafe.alloc[TSQueryCapture](1)
  ...
  opaque type TSQueryMatch =
    CStruct4[uint32_t, uint16_t, uint16_t, Ptr[TSQueryCapture]]

  object TSQueryMatch:
    given _tag: Tag[TSQueryMatch] = Tag
      .materializeCStruct4Tag[uint32_t, uint16_t, uint16_t, Ptr[TSQueryCapture]]

    // Allocates TSQueryMatch on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSQueryMatch] =
      scala.scalanative.unsafe.alloc[TSQueryMatch](1)
    def apply(
        id: uint32_t,
        pattern_index: uint16_t,
        capture_count: uint16_t,
        captures: Ptr[TSQueryCapture]
    )(using Zone): Ptr[TSQueryMatch] =
      val ____ptr = apply()
      (!____ptr).id = id
  ...
  opaque type TSQueryPredicateStep =
    CStruct2[TSQueryPredicateStepType, uint32_t]

  object TSQueryPredicateStep:
    given _tag: Tag[TSQueryPredicateStep] =
      Tag.materializeCStruct2Tag[TSQueryPredicateStepType, uint32_t]

    // Allocates TSQueryPredicateStep on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSQueryPredicateStep] =
      scala.scalanative.unsafe.alloc[TSQueryPredicateStep](1)
    def apply(`type`: TSQueryPredicateStepType, value_id: uint32_t)(using
        Zone
  ...
  end TSQueryPredicateStep

  opaque type TSRange = CStruct4[TSPoint, TSPoint, uint32_t, uint32_t]

  object TSRange:
    given _tag: Tag[TSRange] =
      Tag.materializeCStruct4Tag[TSPoint, TSPoint, uint32_t, uint32_t]

    // Allocates TSRange on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSRange] =
      scala.scalanative.unsafe.alloc[TSRange](1)
    def apply(
        start_point: TSPoint,
        end_point: TSPoint,
        start_byte: uint32_t,
        end_byte: uint32_t
    )(using Zone): Ptr[TSRange] =
      val ____ptr = apply()
      (!____ptr).start_point = start_point
  ...
  opaque type TSTreeCursor =
    CStruct3[Ptr[Byte], Ptr[Byte], CArray[uint32_t, Nat._2]]

  object TSTreeCursor:
    given _tag: Tag[TSTreeCursor] =
      Tag.materializeCStruct3Tag[Ptr[Byte], Ptr[Byte], CArray[uint32_t, Nat._2]]

    // Allocates TSTreeCursor on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSTreeCursor] =
      scala.scalanative.unsafe.alloc[TSTreeCursor](1)
    def apply(
        tree: Ptr[Byte],
        id: Ptr[Byte],
        context: CArray[uint32_t, Nat._2]
    )(using Zone): Ptr[TSTreeCursor] =
      val ____ptr = apply()
      (!____ptr).tree = tree
      (!____ptr).id = id
      (!____ptr).context = context
  ...
  object TSWasmError:
    given _tag: Tag[TSWasmError] =
      Tag.materializeCStruct2Tag[TSWasmErrorKind, CString]

    // Allocates TSWasmError on the heap – fields are not initalised or zeroed out
    def apply()(using Zone): Ptr[TSWasmError] =
      scala.scalanative.unsafe.alloc[TSWasmError](1)
    def apply(kind: TSWasmErrorKind, message: CString)(using
        Zone
    ): Ptr[TSWasmError] =
      val ____ptr = apply()
      (!____ptr).kind = kind
...
private[treesitter] object extern_functions:
  ...
  import _root_.treesitter.aliases.*
  import _root_.treesitter.structs.*
  private[treesitter] def __sn_wrap_treesitter_ts_node_child(
      self: Ptr[TSNode],
      child_index: uint32_t,
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_child_by_field_id(
      self: Ptr[TSNode],
      field_id: TSFieldId,
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_child_by_field_name(
      self: Ptr[TSNode],
      name: CString,
  ...
  private[treesitter] def __sn_wrap_treesitter_ts_node_child_count(
      self: Ptr[TSNode]
  ): uint32_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_descendant_count(
      self: Ptr[TSNode]
  ): uint32_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_descendant_for_byte_range(
      self: Ptr[TSNode],
      start: uint32_t,
      end: uint32_t,
      __return: Ptr[TSNode]
  ...
  private[treesitter] def __sn_wrap_treesitter_ts_node_descendant_for_point_range(
      self: Ptr[TSNode],
      start: Ptr[TSPoint],
      end: Ptr[TSPoint],
      __return: Ptr[TSNode]
  ...
  private[treesitter] def __sn_wrap_treesitter_ts_node_end_byte(
      self: Ptr[TSNode]
  ): uint32_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_end_point(
      self: Ptr[TSNode],
      __return: Ptr[TSPoint]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_eq(
      self: Ptr[TSNode],
      other: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_field_name_for_child(
      self: Ptr[TSNode],
      child_index: uint32_t
  ): CString = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_first_child_for_byte(
      self: Ptr[TSNode],
      byte: uint32_t,
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_first_named_child_for_byte(
      self: Ptr[TSNode],
      byte: uint32_t,
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_grammar_symbol(
      self: Ptr[TSNode]
  ): TSSymbol = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_grammar_type(
      self: Ptr[TSNode]
  ): CString = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_has_changes(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_has_error(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_is_error(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_is_extra(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_is_missing(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_is_named(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_is_null(
      self: Ptr[TSNode]
  ): Boolean = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_language(
      self: Ptr[TSNode]
  ): Ptr[TSLanguage] = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_named_child(
      self: Ptr[TSNode],
      child_index: uint32_t,
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_named_child_count(
      self: Ptr[TSNode]
  ): uint32_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_named_descendant_for_byte_range(
      self: Ptr[TSNode],
      start: uint32_t,
      end: uint32_t,
      __return: Ptr[TSNode]
  ...
  private[treesitter] def __sn_wrap_treesitter_ts_node_named_descendant_for_point_range(
      self: Ptr[TSNode],
      start: Ptr[TSPoint],
      end: Ptr[TSPoint],
      __return: Ptr[TSNode]
  ...
  private[treesitter] def __sn_wrap_treesitter_ts_node_next_named_sibling(
      self: Ptr[TSNode],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_next_parse_state(
      self: Ptr[TSNode]
  ): TSStateId = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_next_sibling(
      self: Ptr[TSNode],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_parent(
      self: Ptr[TSNode],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_parse_state(
      self: Ptr[TSNode]
  ): TSStateId = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_prev_named_sibling(
      self: Ptr[TSNode],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_prev_sibling(
      self: Ptr[TSNode],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_start_byte(
      self: Ptr[TSNode]
  ): uint32_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_start_point(
      self: Ptr[TSNode],
      __return: Ptr[TSPoint]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_string(
      self: Ptr[TSNode]
  ): CString = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_symbol(
      self: Ptr[TSNode]
  ): TSSymbol = extern

  private[treesitter] def __sn_wrap_treesitter_ts_node_type(
      self: Ptr[TSNode]
  ): CString = extern

  private[treesitter] def __sn_wrap_treesitter_ts_parser_logger(
      self: Ptr[TSParser],
      __return: Ptr[TSLogger]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_parser_parse(
      self: Ptr[TSParser],
      old_tree: Ptr[TSTree],
      input: Ptr[TSInput]
  ): Ptr[TSTree] = extern

  private[treesitter] def __sn_wrap_treesitter_ts_parser_set_logger(
      self: Ptr[TSParser],
      logger: Ptr[TSLogger]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_query_cursor_exec(
      self: Ptr[TSQueryCursor],
      query: Ptr[TSQuery],
      node: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_query_cursor_set_point_range(
      self: Ptr[TSQueryCursor],
      start_point: Ptr[TSPoint],
      end_point: Ptr[TSPoint]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_cursor_copy(
      cursor: Ptr[TSTreeCursor],
      __return: Ptr[TSTreeCursor]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_cursor_current_node(
      self: Ptr[TSTreeCursor],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_cursor_goto_first_child_for_point(
      self: Ptr[TSTreeCursor],
      goal_point: Ptr[TSPoint]
  ): int64_t = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_cursor_new(
      node: Ptr[TSNode],
      __return: Ptr[TSTreeCursor]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_cursor_reset(
      self: Ptr[TSTreeCursor],
      node: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_root_node(
      self: Ptr[TSTree],
      __return: Ptr[TSNode]
  ): Unit = extern

  private[treesitter] def __sn_wrap_treesitter_ts_tree_root_node_with_offset(
      self: Ptr[TSTree],
      offset_bytes: uint32_t,
      offset_extent: Ptr[TSPoint],
      __return: Ptr[TSNode]
  ): Unit = extern
  ...
  /** Get the numerical id for the given field name string.
    */
  def ts_language_field_id_for_name(
      self: Ptr[TSLanguage],
      name: CString,
      name_length: uint32_t
  ): TSFieldId = extern
  ...
  /** Get the field name string for the given numerical id.
    */
  def ts_language_field_name_for_id(
      self: Ptr[TSLanguage],
      id: TSFieldId
  ): CString = extern
  ...
  /** Get the numerical id for the given node type string.
    */
  def ts_language_symbol_for_name(
      self: Ptr[TSLanguage],
      string: CString,
      length: uint32_t,
      is_named: Boolean
  ...
  /** Get the current symbol of the lookahead iterator;
    */
  def ts_lookahead_iterator_current_symbol(
      self: Ptr[TSLookaheadIterator]
  ): TSSymbol = extern

  /** Get the current symbol type of the lookahead iterator as a null terminated
    * string.
    */
  def ts_lookahead_iterator_current_symbol_name(
      self: Ptr[TSLookaheadIterator]
  ): CString = extern

  /** Delete a lookahead iterator freeing all the memory used.
    */
  def ts_lookahead_iterator_delete(self: Ptr[TSLookaheadIterator]): Unit =
    extern

  /** Get the current language of the lookahead iterator.
    */
  def ts_lookahead_iterator_language(
      self: Ptr[TSLookaheadIterator]
  ): Ptr[TSLanguage] = extern

  /** Create a new lookahead iterator for the given language and parse state.
    */
  def ts_lookahead_iterator_new(
      self: Ptr[TSLanguage],
      state: TSStateId
  ): Ptr[TSLookaheadIterator] = extern

  /** Advance the lookahead iterator to the next symbol.
    */
  def ts_lookahead_iterator_next(self: Ptr[TSLookaheadIterator]): Boolean =
    extern

  /** Reset the lookahead iterator.
    */
  def ts_lookahead_iterator_reset(
      self: Ptr[TSLookaheadIterator],
  ...
  /** Reset the lookahead iterator to another state.
    */
  def ts_lookahead_iterator_reset_state(
      self: Ptr[TSLookaheadIterator],
      state: TSStateId
  ): Boolean = extern
  ...
  /** Use the parser to parse some source code stored in one contiguous buffer
    ...
    */
  def ts_parser_parse_string_encoding(
      self: Ptr[TSParser],
      old_tree: Ptr[TSTree],
      string: CString,
      length: uint32_t,
  ...
  /** Set the parser's current cancellation flag pointer.
    */
  def ts_parser_set_cancellation_flag(
      self: Ptr[TSParser],
      flag: Ptr[size_t]
  ): Unit = extern
  ...
  /** Set the ranges of text that the parser should include when parsing.
    */
  def ts_parser_set_included_ranges(
      self: Ptr[TSParser],
      ranges: Ptr[TSRange],
      count: uint32_t
  ): Boolean = extern
  ...
  /** Set the maximum duration in microseconds that parsing should be allowed to
    * take before halting.
    */
  def ts_parser_set_timeout_micros(
      self: Ptr[TSParser],
      timeout_micros: uint64_t
  ): Unit = extern

  /** Assign the given Wasm store to the parser. A parser must have a Wasm store
    * in order to use Wasm languages.
    */
  def ts_parser_set_wasm_store(_0: Ptr[TSParser], _1: Ptr[TSWasmStore]): Unit =
    extern
  ...
  /** Get the name and length of one of the query's captures, or one of the
    ...
    */
  def ts_query_capture_name_for_id(
      self: Ptr[TSQuery],
      index: uint32_t,
      length: Ptr[uint32_t]
  ): CString = extern
  ...
  /** Get the quantifier of the query's captures. Each capture is * associated
    ...
    * source.
    */
  def ts_query_capture_quantifier_for_id(
      self: Ptr[TSQuery],
  ...
  /** Manage the maximum number of in-progress matches allowed by this query
    * cursor.
    */
  def ts_query_cursor_did_exceed_match_limit(
      self: Ptr[TSQueryCursor]
  ): Boolean = extern
  ...
  /** Advance to the next capture of the currently running query.
    */
  def ts_query_cursor_next_capture(
      self: Ptr[TSQueryCursor],
      `match`: Ptr[TSQueryMatch],
      capture_index: Ptr[uint32_t]
  ): Boolean = extern

  /** Advance to the next match of the currently running query.
    */
  def ts_query_cursor_next_match(
      self: Ptr[TSQueryCursor],
      `match`: Ptr[TSQueryMatch]
  ): Boolean = extern

  def ts_query_cursor_remove_match(
      self: Ptr[TSQueryCursor],
      match_id: uint32_t
  ): Unit = extern
  ...
  /** Set the range of bytes or (row, column) positions in which the query will
    * be executed.
    */
  def ts_query_cursor_set_byte_range(
      self: Ptr[TSQueryCursor],
      start_byte: uint32_t,
      end_byte: uint32_t
  ): Unit = extern

  def ts_query_cursor_set_match_limit(
      self: Ptr[TSQueryCursor],
      limit: uint32_t
  ): Unit = extern
  ...
  /** Set the maximum start depth for a query cursor.
    */
  def ts_query_cursor_set_max_start_depth(
      self: Ptr[TSQueryCursor],
  ...
  /** Disable a certain pattern within a query.
    */
  def ts_query_disable_pattern(
      self: Ptr[TSQuery],
      pattern_index: uint32_t
  ): Unit = extern

  def ts_query_is_pattern_guaranteed_at_step(
      self: Ptr[TSQuery],
      byte_offset: uint32_t
  ): Boolean = extern

  def ts_query_is_pattern_non_local(
      self: Ptr[TSQuery],
      pattern_index: uint32_t
  ): Boolean = extern

  def ts_query_is_pattern_rooted(
      self: Ptr[TSQuery],
      pattern_index: uint32_t
  ): Boolean = extern
  ...
  /** Get all of the predicates for the given pattern in the query.
    */
  def ts_query_predicates_for_pattern(
      self: Ptr[TSQuery],
      pattern_index: uint32_t,
      step_count: Ptr[uint32_t]
  ): Ptr[TSQueryPredicateStep] = extern

  /** Get the byte offset where the given pattern starts in the query's source.
    */
  def ts_query_start_byte_for_pattern(
      self: Ptr[TSQuery],
      pattern_index: uint32_t
  ): uint32_t = extern
  ...
  def ts_query_string_value_for_id(
      self: Ptr[TSQuery],
      index: uint32_t,
      length: Ptr[uint32_t]
  ): CString = extern
  ...
  /** Set the allocation functions used by the library.
    */
  def ts_set_allocator(
      new_malloc: CFuncPtr1[size_t, Ptr[Byte]],
      new_calloc: CFuncPtr2[size_t, size_t, Ptr[Byte]],
      new_realloc: CFuncPtr2[Ptr[Byte], size_t, Ptr[Byte]],
      new_free: CFuncPtr1[Ptr[Byte], Unit]
  ): Unit = extern
  ...
  /** Get the index of the cursor's current node out of all of the descendants
    ...
    */
  def ts_tree_cursor_current_descendant_index(
      self: Ptr[TSTreeCursor]
  ): uint32_t = extern

  /** Get the field id of the tree cursor's current node.
    */
  def ts_tree_cursor_current_field_id(self: Ptr[TSTreeCursor]): TSFieldId =
    extern

  /** Get the field name of the tree cursor's current node.
    */
  def ts_tree_cursor_current_field_name(self: Ptr[TSTreeCursor]): CString =
    extern
  ...
  /** Move the cursor to the node that is the nth descendant of the original
    * node that the cursor was constructed with, where zero represents the
    * original node itself.
    */
  def ts_tree_cursor_goto_descendant(
      self: Ptr[TSTreeCursor],
  ...
  /** Move the cursor to the first child of its current node that extends beyond
    ...
    */
  def ts_tree_cursor_goto_first_child_for_byte(
      self: Ptr[TSTreeCursor],
      goal_byte: uint32_t
  ): int64_t = extern
  ...
  /** Move the cursor to the next sibling of its current node.
    */
  def ts_tree_cursor_goto_next_sibling(self: Ptr[TSTreeCursor]): Boolean =
    extern
  ...
  /** Move the cursor to the previous sibling of its current node.
    */
  def ts_tree_cursor_goto_previous_sibling(self: Ptr[TSTreeCursor]): Boolean =
    extern

  /** Re-initialize a tree cursor to the same position as another cursor.
    */
  def ts_tree_cursor_reset_to(
      dst: Ptr[TSTreeCursor],
      src: Ptr[TSTreeCursor]
  ): Unit = extern
  ...
  /** Compare an old edited syntax tree to a new syntax tree representing the
    ...
    * changed.
    */
  def ts_tree_get_changed_ranges(
      old_tree: Ptr[TSTree],
      new_tree: Ptr[TSTree],
      length: Ptr[uint32_t]
  ): Ptr[TSRange] = extern
  ...
  /** Write a DOT graph describing the syntax tree to the given file.
    */
  def ts_tree_print_dot_graph(self: Ptr[TSTree], file_descriptor: CInt): Unit =
    extern
  ...
  /** Create a language from a buffer of Wasm. The resulting language behaves
    ...
    */
  def ts_wasm_store_load_language(
      _0: Ptr[TSWasmStore],
      name: CString,
      wasm: CString,
      wasm_len: uint32_t,
      error: Ptr[TSWasmError]
  ): Ptr[TSLanguage] = extern

  /** Create a Wasm store.
    */
  def ts_wasm_store_new(
      engine: Ptr[TSWasmEngine],
      error: Ptr[TSWasmError]
  ): Ptr[TSWasmStore] = extern
...
object functions:
  ...
  def ts_node_child(self: TSNode, child_index: uint32_t)(using Zone): TSNode =
    ...
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_child(
      (__ptr_0 + 0),
      child_index,
      (__ptr_0 + 1)
    )
  ...
  /** Get the node's child at the given index, where zero represents the first
    * child.
    */
  def ts_node_child(self: Ptr[TSNode], child_index: uint32_t)(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  /** Get the node's child at the given index, where zero represents the first
    * child.
    */
  def ts_node_child(self: Ptr[TSNode], child_index: uint32_t)(using
      Zone
  ): TSNode =
  ...
  /** Get the node's child with the given numerical field id.
    */
  def ts_node_child_by_field_id(self: Ptr[TSNode], field_id: TSFieldId)(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  /** Get the node's child with the given numerical field id.
    */
  def ts_node_child_by_field_id(self: Ptr[TSNode], field_id: TSFieldId)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    __sn_wrap_treesitter_ts_node_child_by_field_id(
      self,
      field_id,
      (__ptr_0 + 0)
    )
    !(__ptr_0 + 0)
  end ts_node_child_by_field_id

  /** Get the node's child with the given numerical field id.
    */
  def ts_node_child_by_field_id(self: TSNode, field_id: TSFieldId)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
  ...
  /** Get the node's child with the given field name.
    */
  def ts_node_child_by_field_name(
      self: Ptr[TSNode],
      name: CString,
      name_length: uint32_t
  )(__return: Ptr[TSNode]): Unit =
    __sn_wrap_treesitter_ts_node_child_by_field_name(
      self,
      name,
      name_length,
      __return
  ...
  def ts_node_child_by_field_name(
    ...
    !(__ptr_0 + 0)
  end ts_node_child_by_field_name
  ...
  /** Get the smallest node within this node that spans the given range of bytes
    ...
    */
  def ts_node_descendant_for_byte_range(
      self: TSNode,
      start: uint32_t,
      end: uint32_t
  )(using Zone): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_descendant_for_byte_range(
      (__ptr_0 + 0),
      start,
      end,
      (__ptr_0 + 1)
    ...
    !(__ptr_0 + 1)
  end ts_node_descendant_for_byte_range
  ...
  def ts_node_descendant_for_point_range(
      self: Ptr[TSNode],
      start: Ptr[TSPoint],
      end: Ptr[TSPoint]
  )(using Zone): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    __sn_wrap_treesitter_ts_node_descendant_for_point_range(
      self,
      start,
      end,
      (__ptr_0 + 0)
    ...
    !(__ptr_0 + 0)
  end ts_node_descendant_for_point_range
  ...
  def ts_node_descendant_for_point_range(
    ...
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    val __ptr_1: Ptr[TSPoint] =
      _root_.scala.scalanative.unsafe.alloc[TSPoint](2)
    !(__ptr_0 + 0) = self
    !(__ptr_1 + 0) = start
    !(__ptr_1 + 1) = end
    __sn_wrap_treesitter_ts_node_descendant_for_point_range(
  ...
  def ts_node_end_point(self: Ptr[TSNode])(using Zone): TSPoint =
    val __ptr_0: Ptr[TSPoint] =
      _root_.scala.scalanative.unsafe.alloc[TSPoint](1)
  ...
  /** Get the field name for node's child at the given index, where zero
    ...
    */
  def ts_node_field_name_for_child(self: TSNode, child_index: uint32_t)(using
      Zone
  ): CString =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_field_name_for_child(
      (__ptr_0 + 0),
      child_index
    )
  ...
  /** Get the field name for node's child at the given index, where zero
    * represents the first child. Returns NULL, if no field is found.
    */
  def ts_node_field_name_for_child(
      self: Ptr[TSNode],
      child_index: uint32_t
  ): CString =
  ...
  /** Get the node's first child that extends beyond the given byte offset.
    */
  def ts_node_first_child_for_byte(self: Ptr[TSNode], byte: uint32_t)(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  /** Get the node's first child that extends beyond the given byte offset.
    */
  def ts_node_first_child_for_byte(self: Ptr[TSNode], byte: uint32_t)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
  ...
  /** Get the node's first child that extends beyond the given byte offset.
    */
  def ts_node_first_child_for_byte(self: TSNode, byte: uint32_t)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_first_child_for_byte(
      (__ptr_0 + 0),
      byte,
      (__ptr_0 + 1)
    )
    !(__ptr_0 + 1)
  end ts_node_first_child_for_byte

  /** Get the node's first named child that extends beyond the given byte
    * offset.
    */
  def ts_node_first_named_child_for_byte(self: TSNode, byte: uint32_t)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_first_named_child_for_byte(
      (__ptr_0 + 0),
      byte,
      (__ptr_0 + 1)
    )
    !(__ptr_0 + 1)
  end ts_node_first_named_child_for_byte

  /** Get the node's first named child that extends beyond the given byte
    * offset.
    */
  def ts_node_first_named_child_for_byte(self: Ptr[TSNode], byte: uint32_t)(
      using Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    __sn_wrap_treesitter_ts_node_first_named_child_for_byte(
  ...
  /** Get the node's *named* child at the given index.
    */
  def ts_node_named_child(self: TSNode, child_index: uint32_t)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_named_child(
      (__ptr_0 + 0),
      child_index,
      (__ptr_0 + 1)
    )
    !(__ptr_0 + 1)
  end ts_node_named_child

  /** Get the node's *named* child at the given index.
    */
  def ts_node_named_child(self: Ptr[TSNode], child_index: uint32_t)(using
      Zone
  ): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
  ...
  /** Get the node's *named* child at the given index.
    */
  def ts_node_named_child(self: Ptr[TSNode], child_index: uint32_t)(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  /** Get the smallest named node within this node that spans the given range of
    ...
    */
  def ts_node_named_descendant_for_byte_range(
      self: Ptr[TSNode],
      start: uint32_t,
      end: uint32_t
  )(__return: Ptr[TSNode]): Unit =
    __sn_wrap_treesitter_ts_node_named_descendant_for_byte_range(
      self,
      start,
      end,
      __return
  ...
  def ts_node_named_descendant_for_byte_range(
    ...
    !(__ptr_0 + 0)
  end ts_node_named_descendant_for_byte_range
  ...
  def ts_node_named_descendant_for_point_range(
      self: Ptr[TSNode],
      start: Ptr[TSPoint],
      end: Ptr[TSPoint]
  )(__return: Ptr[TSNode]): Unit =
    __sn_wrap_treesitter_ts_node_named_descendant_for_point_range(
      self,
      start,
      end,
      __return
  ...
  def ts_node_named_descendant_for_point_range(
    ...
    !(__ptr_0 + 0)
  end ts_node_named_descendant_for_point_range
  ...
  /** Get the node's next / previous *named* sibling.
    */
  def ts_node_next_named_sibling(self: Ptr[TSNode])(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  def ts_node_next_named_sibling(self: TSNode)(using Zone): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_next_named_sibling(
      (__ptr_0 + 0),
      (__ptr_0 + 1)
    )
    !(__ptr_0 + 1)
  ...
  def ts_node_prev_named_sibling(self: TSNode)(using Zone): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](2)
    !(__ptr_0 + 0) = self
    __sn_wrap_treesitter_ts_node_prev_named_sibling(
      (__ptr_0 + 0),
      (__ptr_0 + 1)
    )
    !(__ptr_0 + 1)

  def ts_node_prev_named_sibling(self: Ptr[TSNode])(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  def ts_parser_logger(self: Ptr[TSParser])(using Zone): TSLogger =
    val __ptr_0: Ptr[TSLogger] =
      _root_.scala.scalanative.unsafe.alloc[TSLogger](1)
  ...
  def ts_parser_parse(
      ...
      old_tree: Ptr[TSTree],
      input: TSInput
  )(using Zone): Ptr[TSTree] =
    val __ptr_0: Ptr[TSInput] =
      _root_.scala.scalanative.unsafe.alloc[TSInput](1)
    !(__ptr_0 + 0) = input
    __sn_wrap_treesitter_ts_parser_parse(self, old_tree, (__ptr_0 + 0))

  /** Set the logger that a parser should use during parsing.
    */
  def ts_parser_set_logger(self: Ptr[TSParser], logger: TSLogger)(using
      Zone
  ): Unit =
    val __ptr_0: Ptr[TSLogger] =
      _root_.scala.scalanative.unsafe.alloc[TSLogger](1)
  ...
  def ts_query_cursor_set_point_range(
      self: Ptr[TSQueryCursor],
      start_point: Ptr[TSPoint],
      end_point: Ptr[TSPoint]
  ): Unit =
    __sn_wrap_treesitter_ts_query_cursor_set_point_range(
      self,
      start_point,
      end_point
    )
  ...
  end ts_query_cursor_set_point_range

  def ts_tree_cursor_copy(cursor: Ptr[TSTreeCursor])(
      __return: Ptr[TSTreeCursor]
  ): Unit =
  ...
  def ts_tree_cursor_copy(cursor: Ptr[TSTreeCursor])(using Zone): TSTreeCursor =
    val __ptr_0: Ptr[TSTreeCursor] =
      _root_.scala.scalanative.unsafe.alloc[TSTreeCursor](1)
    __sn_wrap_treesitter_ts_tree_cursor_copy(cursor, (__ptr_0 + 0))
    !(__ptr_0 + 0)

  /** Get the tree cursor's current node.
    */
  def ts_tree_cursor_current_node(self: Ptr[TSTreeCursor])(
      __return: Ptr[TSNode]
  ): Unit =
  ...
  def ts_tree_cursor_goto_first_child_for_point(
      self: Ptr[TSTreeCursor],
      goal_point: TSPoint
  )(using Zone): int64_t =
    val __ptr_0: Ptr[TSPoint] =
    ...
    __sn_wrap_treesitter_ts_tree_cursor_goto_first_child_for_point(
      self,
      (__ptr_0 + 0)
    )
  end ts_tree_cursor_goto_first_child_for_point
  ...
  /** Re-initialize a tree cursor to start at a different node.
    */
  def ts_tree_cursor_reset(self: Ptr[TSTreeCursor], node: TSNode)(using
      Zone
  ): Unit =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    !(__ptr_0 + 0) = node
  ...
  /** Get the root node of the syntax tree, but with its position shifted
    ...
    */
  def ts_tree_root_node_with_offset(
      self: Ptr[TSTree],
      offset_bytes: uint32_t,
      offset_extent: Ptr[TSPoint]
  )(using Zone): TSNode =
    val __ptr_0: Ptr[TSNode] = _root_.scala.scalanative.unsafe.alloc[TSNode](1)
    __sn_wrap_treesitter_ts_tree_root_node_with_offset(
      self,
      offset_bytes,
      offset_extent,
      (__ptr_0 + 0)
    ...
    !(__ptr_0 + 0)
  end ts_tree_root_node_with_offset
